...
# Check pipe-fail support in a subshell, ignore if unsupported
# shellcheck disable=SC3040
(set -o pipefail 2> /dev/null) && set -o pipefail

help() {
  cat <<'EOF'
Install a binary release of yak hosted on GitHub
...
url=https://github.com/anadinema/yak
...
if [ -z "${tag-}" ]; then
  tag=$(
    download https://api.github.com/repos/anadinema/yak/releases/latest - |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
archive="$releases/download/$tag/yak-$tag-$target.$extension"
...
if [ "$extension" = "zip" ]; then
  download "$archive" "$td/yak.zip"
  unzip -d "$td" "$td/yak.zip"
else
  download "$archive" - | tar -C "$td" -xz
fi

if [ -e "$dest/yak" ] && [ "$force" = false ]; then
  err "\`$dest/yak\` already exists"
else
  mkdir -p "$dest"
  cp "$td/yak" "$dest/yak"
  chmod 755 "$dest/yak"
